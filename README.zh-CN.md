...
现在项目的主运行路径也已经接入了一个内置的 RestFlow Rust bridge，用来管理 provider/auth/profile 并完成单回合 AI 决策。这样 web UI 里的 AI 座位可以自动落子，不再完全依赖外部 MCP 宿主持续循环。
...
当前 Node game server 仍然是 session、游戏规则、SSE 和 MCP 的唯一真相源。
...
- 内置 AI seat 通过 RestFlow bridge 自动接管某一方
